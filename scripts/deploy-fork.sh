...
# Test deployment on local anvil fork of Polygon Amoy
# First start anvil: anvil --fork-url https://polygon-amoy-bor-rpc.publicnode.com --port 8545
# Usage: ./deploy-fork.sh
...
echo "=== Test Deployment on Anvil Fork ==="
echo "Salt: 0x$SALT"
...
echo "Expected: $OWNER"
# Case-insensitive comparison (addresses may have different checksumming)
if [ "${ACTUAL_OWNER,,}" = "${OWNER,,}" ]; then
    echo "✓ Owner is correct!"
else
    echo "✗ Owner mismatch!"
    exit 1
fi
...
echo ""
echo "=== Fork Test Deployment Complete ==="
echo "Verifier: $VERIFIER_ADDR"
echo "Implementation: $IMPL_ADDR"
echo "Proxy: $PROXY_ADDR"
