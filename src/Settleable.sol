...
abstract contract Settleable is ISettleable, OwnableUpgradeable {
    ...
    function settleAll(string calldata reason) external onlyOwner {
        ...
        // Route any residual balance through the inheritor's labeled _routeDustToTreasury.
        // Under normal operation this is integer-division rounding from claimFromForfeited's
        // cumulative-divisor algorithm — at most a few thousand wei. SELFDESTRUCT-forced ETH
        // also lands here; we route it rather than capping, because reverting on excess
        // would let a 10001-wei attacker permanently brick finalization. Indexers should
        // flag any unusually large `dust` in the Settled event for off-chain reconciliation.
        if (dust > 0) {
