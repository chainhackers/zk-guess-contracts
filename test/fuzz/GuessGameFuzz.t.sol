...
contract GuessGameFuzz is Test {
    ...
    function testFuzz_BountyDistributionDust(uint256 collateral, uint8 numGuessers, uint256 stakeSeed) public {
        ...
        vm.prank(creator);
        uint256 puzzleId = game.createPuzzle{value: MIN_BOUNTY + collateral}(COMMITMENT_42_123, MIN_STAKE, 100);
    ...
    function testFuzz_MultiPuzzleBalanceAccumulation(uint8 numPuzzles, uint256 collateralSeed) public {
        ...
        for (uint256 i = 0; i < numPuzzles; i++) {
            ...
            vm.prank(creator);
            puzzleIds[i] = game.createPuzzle{value: MIN_BOUNTY + collateral}(COMMITMENT_42_123, MIN_STAKE, 100);
    ...
    function testFuzz_BountyShareDivision(uint256 collateral, uint8 numChallenges) public {
        ...
        vm.prank(creator);
        uint256 puzzleId = game.createPuzzle{value: MIN_BOUNTY + collateral}(COMMITMENT_42_123, MIN_STAKE, 1000);
